...
#define LCD_SCL      OLED_SCL_PIN
#define LCD_SDA      OLED_SDA_PIN
#define LCD_SCL_PORT OLED_SCL_PORT
#define LCD_SDA_PORT OLED_SDA_PORT

#define LCD_SDAIN()                                              \
    do {                                                         \
        GPIO_Init(LCD_SDA_PORT, LCD_SDA, GPIO_Mode_In_FL_No_IT); \
    } while (0)
#define LCD_SDAOUT_1()                                               \
    do {                                                             \
        GPIO_Init(LCD_SDA_PORT, LCD_SDA, GPIO_Mode_Out_PP_Low_Fast); \
    } while (0)
#define LCD_SCL_HIGH()                       \
    do {                                     \
...
#define LCD_SCL_LOW()                          \
    do {                                       \
        GPIO_ResetBits(LCD_SCL_PORT, LCD_SCL); \
    } while (0)
#define LCD_SDA_HIGH()                       \
    do {                                     \
...
#define LCD_SDA_LOW()                          \
    do {                                       \
        GPIO_ResetBits(LCD_SDA_PORT, LCD_SDA); \
    } while (0)
...
uint8_t I2C_SendByte(uint8_t Byte)
{
    ...
    LCD_SCL_HIGH();
    I2C_Delay();
    if (GPIO_ReadInputDataBit(LCD_SDA_PORT, LCD_SDA)) {
        F_temp = 1;
    }
    I2C_Delay();
    LCD_SCL_LOW();
...
{
    uint16_t tmp;
    struct TimeType *time = RTC_GetTimeAddr();
